...
var jqSlide = {
  ...
  **/
  init: function(element, options) {
    ...
    if (options) {
      ...
      this.tabLocation = options.tabLocation; 
    }
  ...
  **/
  constructSlider: function(element, headerElements, contentElements) {
    ...
    var tabBar = '<div class="tab-slider ' + this.tabLocation + '" id="slider"><ul class="slide-navbar">';
    $.each(headerElements, function(i, value) {
      tabBar += '<li id="slide-indicator-' + (i + 1) + '" onclick="jqSlide.initiateSlide(' + (i + 1) + ');">' + $(headerElements[i]).html() + '</li>';
    });
    ...
    if (this.tabLocation == "bottom") {
      nodeString = nodeString + tabBar;
    } else {
      nodeString = tabBar + nodeString;
    }
